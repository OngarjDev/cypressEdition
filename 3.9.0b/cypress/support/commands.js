...
function findButton(selector, $element) {
  let $btn = $element.find(`[data-cy=${selector}]`);
  if ($btn.length === 0) {
    $btn = $element.parent().find(`[data-cy=${selector}]`).first();
    if ($btn.length === 0) {
      $btn = $element.parent().parent().find(`[data-cy=${selector}]`).first();
    }
  }
  return $btn;
}
...
Cypress.Commands.add('findButton', ($element, selector) => {
  const $btn = findButton(selector, $element);
  cy.wrap($btn);
});


function assertHiddenOrDisabled($btn, label, expectedText) {
  ...
  if (isVisible) {
    ...
    if (expectedText) {
      expect($btn.text().trim(), `${label} should have text "${expectedText}"`).to.equal(expectedText);
    }
  } else {
...
Cypress.Commands.add('shouldBeHiddenOrDisabled', 
  { prevSubject: 'element' }, ($el, label = 'Element', expectedText = null) => {
    assertHiddenOrDisabled($el, label, expectedText);
});
