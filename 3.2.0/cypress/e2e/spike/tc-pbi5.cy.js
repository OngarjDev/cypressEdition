describe(`TC-PBI5 : CHANGE-PLAN`, () => {
    ...
    it(`TC-PBI5-1 : 'Change' button – behavior`,()=>{    
        ...
        cy.log('* ') ;
        cy.log('* Step 2: Sign-in as 67130500173 (Sompong Pordee)');
            cy.signIn('67130500173', 'itbangmod') ;
            // send a request, mock 404 when getting 200 to test change buttton
            cy.intercept({method: 'GET', url: '**/students/67130500173/declared-plan', times: 1}, (req) => {
                req.continue((res) => {
                    if (res.statusCode === 200) {
                        ...
                        res.send({
                            ...
                            body: {"error": "DECLARED_PLAN_NOT_FOUND", 
                                   "message": "No declared plan found for student with id=67130500173."}
                            }) ; } });
        ...
        cy.log('* Step 4: Refresh the page');        
            ...
            cy.intercept({method: 'GET', url: '**/students/67130500173/declared-plan', times: 1}, (req) => {
                req.reply({
                    statusCode: 200,
                    body: { "studentId": "67130500173", "planId": 4, "status": "DECLARED", "createdAt": "2024-01-15T10:00:00Z", "updatedAt": "2024-01-15T10:00:00Z"}
    ...
    it(`TC-PBI5-2 : Change plan twice – success`,()=>{    
        ...
        cy.log('* Step 2: Click Change button') ;
            ...
            cy.intercept('PUT', '**/students/67130500173/declared-plan', (req) => {
        ...
        cy.log('* Step 3: Click OK to close the dialog') ;
            ...
            cy.get('.ecors-declared-plan').invoke('text').as('statusAfterChange1').then( text => Cypress.utils.shouldBeNow(text, { withinMs: 3000 + Cypress.config('regularWaitMs') })) ;
            cy.get('.ecors-dropdown-plan option:selected').should('have.text', 'DB - Database Administrator') ;   
            cy.shouldBeHiddenOrNotExist('.ecors-button-declare') ;
            cy.shouldBeVisibleAndNotClickable('.ecors-button-change') ;
        ...
        cy.log('* Step 5: Click Change button') ;
            ...
            cy.get('.ecors-declared-plan').invoke('text').as('statusAfterChange2').then( text => Cypress.utils.shouldBeNow(text, { withinMs: 3000 + Cypress.config('regularWaitMs') })) ;
            cy.get('.ecors-dropdown-plan option:selected').should('have.text', 'DE - Data Engineer') ;
            cy.shouldBeHiddenOrNotExist('.ecors-button-declare') ;
            cy.shouldBeVisibleAndNotClickable('.ecors-button-change') ;
    ...
    it(`TC-PBI5-3 : Handle 404 Not Found when changing plan`,()=>{    
        ...
        cy.log('* step 0: Sign-in as 67130500173 (Sompong Pordee)');
        cy.signIn('67130500173', 'itbangmod') ;
        cy.visit('reserve.html') ;
        cy.wait(Cypress.config('regularWaitMs')) ;
        ...
        cy.log('* Step 2: Mock a 404 Not Found response for PUT /declared-plan') ;
            ...
            cy.intercept('GET', '**/students/67130500173/declared-plan', (req) => {
                req.reply({
                    statusCode: 404,
                    body: {"error": "DECLARED_PLAN_NOT_FOUND", 
                        "message": "No declared plan found for student with id=67130500173."}
        ...
        cy.log('* Step 3: Click Change button') ;
            ...
            cy.wait(Cypress.config('regularWaitMs')) ;
            cy.shouldShowDialog("No declared plan found for student with id=67130500173.");
